...
def api_return_product():
    ...
    try:
        ...
        if total_returned_usd > 0:
            ...
            # To'lovlarni yangilash (KASKAD USULI)
            returned_uzs = total_returned_usd * sale.currency_rate
            
            # 1. Naqd to'lovlarni topish
            cash_payments = DebtPayment.query.filter_by(
                sale_id=sale_id,
                payment_type='cash'
            ).order_by(DebtPayment.id.desc()).all()
            
            total_cash = sum(Decimal(str(p.amount_usd)) for p in cash_payments)
            logger.info(f"Jami naqd to'lovlar: ${total_cash}")
            
            remaining_to_refund = total_returned_usd
            
            if total_cash >= total_returned_usd:
                # HOLAT 1: Naqd pul yetarli - faqat naqd puldan kamaytiramiz
                logger.info(f"Naqd pul yetarli: ${total_cash} >= ${total_returned_usd}")
                for payment in cash_payments:
                    if remaining_to_refund <= 0:
                        break
                    
                    payment_usd = Decimal(str(payment.amount_usd))
                    deduct_usd = min(payment_usd, remaining_to_refund)
                    deduct_uzs = deduct_usd * sale.currency_rate
                    
                    payment.amount_usd = float(payment_usd - deduct_usd)
                    payment.amount_uzs = float(Decimal(str(payment.amount_uzs)) - deduct_uzs)
                    
                    logger.info(f"Naqd to'lovdan kamaytirildi: ${deduct_usd} (qoldi: ${payment.amount_usd})")
                    remaining_to_refund -= deduct_usd
                
                # Qaytarish yozuvi (tarix uchun)
                refund_payment = DebtPayment(
                    sale_id=sale_id,
                    customer_id=sale.customer_id,
                    payment_type='cash_refund',
                    amount_usd=float(-total_returned_usd),
                    amount_uzs=float(-returned_uzs),
                    payment_date=datetime.now(),
                    description=f'Mahsulot qaytarildi: {len(returned_items)} ta',
                    created_by=session.get('user_id')
                )
                db.session.add(refund_payment)
                logger.info(f"Qaytarish yozuvi qo'shildi: -${total_returned_usd}")
                
            else:
                # HOLAT 2: Naqd pul yetmaydi - kaskad usuli
                logger.info(f"Naqd pul yetmaydi: ${total_cash} < ${total_returned_usd}")
                
                # 2.1: Avval barcha naqd pulni 0 ga tushiramiz
                if total_cash > 0:
                    for payment in cash_payments:
                        logger.info(f"Naqd to'lov 0 ga tushirildi: ${payment.amount_usd}")
                        payment.amount_usd = 0
                        payment.amount_uzs = 0
                    
                    # Naqd qaytarish yozuvi
                    refund_payment = DebtPayment(
                        sale_id=sale_id,
                        customer_id=sale.customer_id,
                        payment_type='cash_refund',
                        amount_usd=float(-total_cash),
                        amount_uzs=float(-total_cash * sale.currency_rate),
                        payment_date=datetime.now(),
                        description=f'Mahsulot qaytarildi (naqd qismi)',
                        created_by=session.get('user_id')
                    )
                    db.session.add(refund_payment)
                    
                    remaining_to_refund = total_returned_usd - total_cash
                    logger.info(f"Naqd puldan ${total_cash} qaytarildi, qoldi: ${remaining_to_refund}")
                
                # 2.2: Qolgan summani boshqa to'lovlardan proporsional kamaytiramiz
                if remaining_to_refund > 0:
                    other_payments = DebtPayment.query.filter(
                        DebtPayment.sale_id == sale_id,
                        DebtPayment.payment_type.in_(['click', 'terminal', 'payme', 'bank_transfer'])
                    ).all()
                    
                    if other_payments:
                        total_other = sum(Decimal(str(p.amount_usd)) for p in other_payments)
                        logger.info(f"Boshqa to'lovlardan kamaytiriladi: ${remaining_to_refund} (jami: ${total_other})")
                        
                        for payment in other_payments:
                            payment_usd = Decimal(str(payment.amount_usd))
                            ratio = payment_usd / total_other
                            deduct_usd = remaining_to_refund * ratio
                            deduct_uzs = deduct_usd * sale.currency_rate
                            
                            payment.amount_usd = float(payment_usd - deduct_usd)
                            payment.amount_uzs = float(Decimal(str(payment.amount_uzs)) - deduct_uzs)
                            
                            logger.info(f"{payment.payment_type} dan kamaytirildi: ${deduct_usd} (qoldi: ${payment.amount_usd})")
                            
                            # Har bir to'lov turi uchun qaytarish yozuvi
                            refund_payment = DebtPayment(
                                sale_id=sale_id,
                                customer_id=sale.customer_id,
                                payment_type=f'{payment.payment_type}_refund',
                                amount_usd=float(-deduct_usd),
                                amount_uzs=float(-deduct_uzs),
                                payment_date=datetime.now(),
                                description=f'Mahsulot qaytarildi ({payment.payment_type} qismi)',
                                created_by=session.get('user_id')
                            )
                            db.session.add(refund_payment)
